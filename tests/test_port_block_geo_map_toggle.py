...
def test_resolve_port_block_asset_target_prefers_all_sources_variant(tmp_path):
...
def test_port_block_asset_serves_all_sources_geo_map_variant_when_requested(tmp_path, monkeypatch):
    root = tmp_path / "port_block"
    default_map = root / "ufw_plots" / "ufw_geo_map.jpg"
    all_sources_map = root / "ufw_plots" / "ufw_geo_map_all_sources.jpg"
    default_map.parent.mkdir(parents=True)
    ...
    client = app.test_client()
    response = client.get(f"/port-block/ufw_plots/ufw_geo_map.jpg?token={AUTH_TOKEN}&all_sources=1")

    assert response.status_code == 200
    assert response.data == b"all-sources-map"